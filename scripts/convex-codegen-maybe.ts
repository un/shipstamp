...
if (!env.CONVEX_DEPLOYMENT && !env.CONVEX_DEPLOY_KEY) {
  console.log(
    "Skipping Convex codegen (missing CONVEX_DEPLOYMENT/CONVEX_DEPLOY_KEY)."
  );
  process.exit(0);
}
...
const child = spawn("bun", ["x", "convex", "codegen"], {
  cwd: repoRoot,
  env,
  stdio: "inherit"
});
